...
test("the auth schema ships outside the globbed directory", () => {
  ...
  const listed = readdirSync(migrationsDir);
  assert.ok(listed.includes("auth"), "migrations/auth/ must exist as the source copy");
...
test("splitSqlStatements drops comments and keeps quoted semicolons", () => {
  const sql = `
    -- heading
    create table t (id int);
    insert into t (id) values (1); /* keep going */
...
test("splitSqlStatements keeps dollar-quoted bodies intact", () => {
  const sql = `create function f() returns void as $$ begin perform 1; end; $$ language plpgsql;`;
  const stmts = splitSqlStatements(sql);
  assert.equal(stmts.length, 1);
  assert.match(stmts[0], /perform 1; end;/);
});

test("this workspace's migrations split into runnable statements", () => {
  const dir = join(projectRoot(), "migrations");
  const files = readdirSync(dir).filter((n) => n.endsWith(".sql")).sort();
  assert.ok(files.length >= 3, "expected app migrations");
  for (const name of files) {
    const stmts = splitSqlStatements(readFileSync(join(dir, name), "utf8"));
    assert.ok(stmts.length > 0, `${name} produced no statements`);
    for (const stmt of stmts) {
      assert.equal(stmt.includes(";"), false, `${name} left a semicolon in: ${stmt.slice(0, 80)}`);
      assert.match(
        stmt,
        /^(create|alter|insert|update|drop|comment|select)\b/i,
        `${name} unexpected statement: ${stmt.slice(0, 80)}`,
      );
    }
  }
});

test("sanitizeDatabaseUrl strips channel_binding and adds sslmode", () => {
  const neon =
    "postgresql://u:p@ep-x.us-east-2.aws.neon.tech/neondb?sslmode=require&channel_binding=require";
  const out = sanitizeDatabaseUrl(neon);
  assert.equal(out.includes("channel_binding"), false);
  assert.match(out, /sslmode=require/);
  assert.equal(sslForDatabaseUrl(out)?.rejectUnauthorized, false);
});

test("sanitizeDatabaseUrl leaves localhost alone", () => {
  const local = "postgresql://u:p@127.0.0.1:5432/db";
  assert.equal(sanitizeDatabaseUrl(local), local);
  assert.equal(sslForDatabaseUrl(local), undefined);
});
